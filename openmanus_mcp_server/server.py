...
import warnings
warnings.filterwarnings("ignore", category=UserWarning)   # requests urllib3 version warning
...
for _noisy in ("browser_use", "urllib3", "asyncio", "httpx", "httpcore",
               "structlog", "root", "openai", "anthropic"):
...
class TaskManager:
    ...
    def __init__(self, output_base_dir: Optional[str] = None):
        ...
        self.output_base_dir = output_base_dir or str(Path.home() / ".openmanus" / "output")
...
def resolve_path_safely(file_path: str, allowed_dirs: Optional[List[str]] = None) -> Optional[Path]:
    ...
    for allowed_dir in dirs:
        try:
            ...
            if str(resolved).startswith(str(resolved_allowed) + os.sep) or str(resolved) == str(resolved_allowed):
                return resolved
        except (OSError, ValueError):
            continue
...
class OpenManusAgent:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        self.task_manager = TaskManager(
            output_base_dir=self.config.get("output_dir")
        )
        self._running = False
    ...
    async def execute_task(
        ...
        if HAS_OPENMANUS:
            try:
                ...
                def custom_emit(event_type: str, data: Any):
                    ...
                    if event_type == "step_start":
                        ...
                        self.task_manager.update_progress(task.task_id, step, max_steps_data)
                ...
                logger.exception(f"Real OpenManus execution failed for task {task.task_id}")
            ...
            logger.warning("OpenManus core not available, falling back to mock execution.")
    ...
    def _simulate_task_execution(self, task: OpenManusTask) -> str:
        ...
        if any(kw in desc_lower for kw in ["code", "review", "analyze", "inspect", "audit"]):
            analysis_items.append("- **Code Analysis**: Task involves code review or analysis.")
            analysis_items.append("  - Recommended: Use `openmanus_code_execute` for validation")
            analysis_items.append("  - Use `openmanus_file_read` to inspect relevant files")
        if any(kw in desc_lower for kw in ["search", "research", "find", "look up", "web"]):
            analysis_items.append("- **Web Research**: Task involves information gathering.")
            analysis_items.append("  - Use `openmanus_web_search` for live data")
            analysis_items.append("  - Use `openmanus_browser_action` for dynamic content")
        if any(kw in desc_lower for kw in ["file", "read", "write", "create", "generate", "document"]):
            analysis_items.append("- **File Operations**: Task involves file I/O.")
            analysis_items.append("  - Use `openmanus_file_read` / `openmanus_file_write`")
            analysis_items.append("  - Use `openmanus_list_files` for directory inspection")
        if any(kw in desc_lower for kw in ["git", "commit", "branch", "merge", "push", "pull"]):
            analysis_items.append("- **Git Operations**: Task involves version control.")
            analysis_items.append("  - Use `openmanus_git_operation` for all git commands")
        if any(kw in desc_lower for kw in ["test", "unit test", "integration test", "verify"]):
            analysis_items.append("- **Testing**: Task involves testing.")
            analysis_items.append("  - Use `openmanus_code_execute` with Python for pytest")
            analysis_items.append("  - Use `openmanus_code_execute` with JavaScript for Jest")
        if any(kw in desc_lower for kw in ["bug", "fix", "debug", "error", "issue"]):
            analysis_items.append("- **Bug Fix**: Task involves debugging.")
            analysis_items.append("  - Start with `openmanus_file_read` to inspect the code")
        ...
        if any(kw in desc_lower for kw in ["refactor", "improve", "optimize", "clean"]):
            ...
            analysis_items.append("  - Use `openmanus_file_read` to understand current code")
        ...
        if any(kw in desc_lower for kw in ["browser", "web page", "scrape", "crawl"]):
            analysis_items.append("- **Browser Automation**: Task involves web interaction.")
            analysis_items.append("  - Use `openmanus_browser_action` with navigate/click actions")
        ...
        if not analysis_items:
            ...
            analysis_items.append("  - Use `openmanus_file_read` / `openmanus_file_write` for file ops")
...
TOOLS = [
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                "action": {
                    "type": "string",
                    "enum": ["navigate", "click", "fill", "scroll_down", "scroll_up", "scroll_to_text", "send_keys", "get_dropdown_options", "select_dropdown_option", "go_back", "extract_content", "get_current_url", "get_page_text"],
    ...
    Tool(
        ...
        description=(
            "List current resource subscriptions. "
            "Optionally filter by resource URI."
        ),
        inputSchema={
            "type": "object",
            "properties": {
...
async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
    ...
    if name == "openmanus_run_task":
        ...
        if cmd is None:
            return [types.TextContent(type="text", text=f"Error: Unsupported language '{language}'")]

        import tempfile
        if language.lower() in ("python", "python3", "py", "ruby", "rb", "perl"):
            try:
                with tempfile.NamedTemporaryFile(mode='w', suffix=f'.{language.lower()}', delete=False) as f:
                    f.write(code)
                    temp_path = f.name
                cmd = cmd + [temp_path]
            except OSError as e:
                return [types.TextContent(type="text", text=f"Error creating temp file: {e}")]
        ...
        try:
            ...
            try:
                ...
                proc.kill()
                return [types.TextContent(type="text", text=f"Error: Execution timed out after {timeout}s")]
        ...
        if resolved is None:
            return [types.TextContent(type="text", text=f"Error: Path traversal blocked - '{file_path}'")]
        if not resolved.is_file():
            return [types.TextContent(type="text", text=f"Error: Not a file or not found: {resolved}")]
        try:
            ...
            return [types.TextContent(type="text", text=f"Error: Permission denied reading {resolved}")]
        except Exception as e:
        ...
        try:
            ...
            return [types.TextContent(type="text", text=f"Successfully wrote {len(content)} bytes to {resolved}")]
        except PermissionError:
            return [types.TextContent(type="text", text=f"Error: Permission denied writing to {resolved}")]
        except Exception as e:
        ...
        try:
            ...
            req = UReq(url, headers={"User-Agent": "Mozilla/5.0 (compatible; OpenManus/1.0)"})
            ...
            class DDGParser(HTMLParser):
                ...
                def handle_data(self, data):
                    if self.in_title: self.current['title'] = (self.current.get('title','') + data).strip()
                    if self.in_snippet: self.current['snippet'] = (self.current.get('snippet','') + data).strip()
        ...
        return [types.TextContent(type="text", text=f"Error: No search results found for '{query}'")]
        ...
        if not operation:
            return [types.TextContent(type="text", text="Error: No git operation specified")]
        allowed_ops = {"status","diff","log","commit","push","pull","fetch","clone","branch","checkout",
                       "merge","rebase","add","rm","mv","tag","describe","show","grep","blame","annotate",
                       "rev-parse","shortlog","stash","config","remote","init","clean","reset","restore","switch"}
        if operation.lower() not in allowed_ops:
            return [types.TextContent(type="text", text=f"Error: Git operation '{operation}' is not allowed")]
        ...
        try:
            ...
            parts = []
            if stdout: parts.append(stdout.decode('utf-8', errors='replace'))
            if stderr and proc.returncode != 0: parts.append(f"stderr: {stderr.decode('utf-8', errors='replace')}")
            result = "\n".join(parts) or "Command completed (no output)"
            return [types.TextContent(type="text", text=f"Git '{operation}':\n{result}")]
        ...
        try:
            p = Path(path)
            files = [str(f) for f in (p.rglob("*") if recursive else p.iterdir()) if f.is_file()]
            return [types.TextContent(type="text", text="\n".join(files[:100]))]
        except Exception as e:
        ...
        if not resource_uri or not client_id:
            return [types.TextContent(type="text", text="Error: 'resource_uri' and 'client_id' required")]
        return [types.TextContent(type="text", text=subscribe_to_resource(resource_uri, client_id))]
        ...
        if not resource_uri or not client_id:
            return [types.TextContent(type="text", text="Error: 'resource_uri' and 'client_id' required")]
        return [types.TextContent(type="text", text=unsubscribe_from_resource(resource_uri, client_id))]

    elif name == "openmanus_list_subscriptions":
        return [types.TextContent(type="text", text=list_subscriptions(arguments.get("resource_uri")))]
...
_browser_state: Dict[str, Any] = {
    "current_url": "", "page_content": "", "page_html": "", "elements": [],
}
    ...
    try:
        ...
        class TextExtractor(HTMLParser):
            def __init__(self):
                ...
                self._in_skip = False
            def handle_starttag(self, tag, attrs):
                if tag in self._skip: self._in_skip = True
            def handle_endtag(self, tag):
                if tag in self._skip: self._in_skip = False
            def handle_data(self, data):
                if not self._in_skip and data.strip(): self.text.append(data.strip())
        e = TextExtractor()
        e.feed(html)
...
def _extract_clickable_elements(html: str) -> list:
    ...
    for m in re.finditer(r'<a\s+[^>]*href=["\']([^"\']+)["\'][^>]*>([^<]*)</a>', html, re.IGNORECASE):
        elements.append({"text": m.group(2).strip() or "Click here", "href": m.group(1)})
...
async def _handle_browser_action(arguments: dict) -> list[types.TextContent]:
    ...
    if action == "navigate":
        if not url:
            return [types.TextContent(type="text", text="Error: URL required for navigate")]
        try:
            ...
            _browser_state.update({"current_url": url, "page_html": html, "page_content": _extract_text_from_html(html)})
        ...
        prefix = f"Goal: {goal}\n\n" if goal else ""
        return [types.TextContent(type="text", text=prefix + _browser_state["page_content"][:10000])]

    elif action in ("get_current_url",):
        return [types.TextContent(type="text", text=f"Current URL: {_browser_state['current_url'] or 'None'}")]

    elif action == "get_page_text":
        if not _browser_state["page_content"]:
            return [types.TextContent(type="text", text="Error: No page loaded")]
        return [types.TextContent(type="text", text=_browser_state["page_content"][:10000])]
        ...
        if index is not None and 0 <= index < len(elements):
            ...
            return [types.TextContent(type="text", text=f"Clicked: {el.get('text','N/A')} ({href})")]
        return [types.TextContent(type="text", text="Elements:\n" + "\n".join(f"[{i}] {e.get('text')} -> {e.get('href')}" for i, e in enumerate(elements[:20])))]

    elif action == "scroll_to_text":
        if text and text in _browser_state.get("page_content", ""):
            idx = _browser_state["page_content"].index(text)
            ctx = _browser_state["page_content"][max(0,idx-200):idx+len(text)+200]
            return [types.TextContent(type="text", text=f"Found '{text}' at {idx}:\n\n{ctx}")]
        return [types.TextContent(type="text", text=f"Text '{text}' not found")]

    elif action in ("scroll_down", "scroll_up"):
        direction = "down" if action == "scroll_down" else "up"
        return [types.TextContent(type="text", text=f"Scrolled {direction} {abs(scroll_amount) or 100}px (simulated)")]

    elif action == "fill":
        return [types.TextContent(type="text", text=f"Filled '{selector or 'field'}' with '{text}' (simulated)")]

    elif action == "send_keys":
        return [types.TextContent(type="text", text=f"Sent keys '{keys}' (simulated)")]

    elif action == "go_back":
        return [types.TextContent(type="text", text=f"Go back from: {_browser_state['current_url'] or 'None'} (simulated)")]
...
def list_subscriptions(resource_uri: Optional[str] = None) -> str:
    if resource_uri:
        ...
        return json.dumps({"resource": resource_uri, "subscribers": list(subs), "count": len(subs)}, indent=2)
    return json.dumps({u: {"subscribers": list(s), "count": len(s)} for u, s in _resource_subscribers.items()}, indent=2)
...
def get_health_status() -> dict:
    ...
    def _count(s): return sum(1 for t in tasks.values() if t.status == s)
    days = int(uptime // 86400); hours = int((uptime % 86400) // 3600)
    minutes = int((uptime % 3600) // 60); secs = int(uptime % 60)
    parts = ([f"{days}d"] if days else []) + ([f"{hours}h"] if hours else []) + ([f"{minutes}m"] if minutes else []) + [f"{secs}s"]
    return {
        "status": "healthy",
        "server": {"name": "openmanus-mcp-server", "version": "1.0.0", "uptime_seconds": round(uptime, 2), "uptime_human": " ".join(parts)},
        "tasks": {"total": len(tasks), "running": _count(TaskStatus.RUNNING), "pending": _count(TaskStatus.PENDING),
                  "completed": _count(TaskStatus.COMPLETED), "failed": _count(TaskStatus.FAILED), "cancelled": _count(TaskStatus.CANCELLED)},
        "subscriptions": {"resources": len(_resource_subscribers), "total_clients": sum(len(s) for s in _resource_subscribers.values())},
        "browser": {"page_loaded": bool(_browser_state.get("page_content")), "current_url": _browser_state.get("current_url", "")},
...
async def read_resource(uri: str) -> str:
    ...
    if uri == "openmanus://config":
        return json.dumps({"server": {"name": "openmanus-mcp-server", "version": "1.0.0"},
                           "agent": {"max_steps": 30, "model": "gpt-4o"},
                           "security": {"sandbox_enabled": True}}, indent=2)
...
async def get_prompt(name: str, arguments: dict) -> types.GetPromptResult:
    ...
    if name == "code_review":
        return _msg(f"Please review the code in {arguments.get('file_path','unknown')}, focusing on: {arguments.get('focus_areas','general')}")
    elif name == "bug_fix":
        return _msg(f"Please help fix this bug: {arguments.get('bug_description','unknown')} in file {arguments.get('file_path','unknown')}")
    elif name == "feature_impl":
        return _msg(f"Please implement this feature: {arguments.get('feature_description','unknown')} using {arguments.get('tech_stack','default')}")
    elif name == "refactor":
        return _msg(f"Please refactor this code: {arguments.get('code_context','unknown')} with goals: {arguments.get('goals','general improvement')}")
    elif name == "test_gen":
        return _msg(f"Please generate {arguments.get('test_framework','pytest')} tests for:\n\n{arguments.get('code','unknown')}")
    else:
...
async def main():
    ...
    if args.sse:
        ...
        async with sse.connect_sse(fastapi_app, "/sse", "/messages/") as (read_stream, write_stream):
            await app.run(
        ...
        mcp_stdout = anyio.wrap_file(TextIOWrapper(_MCP_STDOUT_BUFFER, encoding="utf-8"))

        async with stdio_server(stdin=mcp_stdin, stdout=mcp_stdout) as (read_stream, write_stream):
            await app.run(
                read_stream, write_stream,
                InitializationOptions(
                    server_name="openmanus-mcp-server",
